...
class Nozzle:
    ...
    def __str__(self):
        return f"Nozzle : D_p = {self.D_p}, D_t = {self.D_t}, D_e = {self.D_e}"

    @property
    def A_p(self):
        """ Plenum chamber area"""
        return math.pi*self.D_p**2/4
...
class ConicalNozzle(Nozzle):
    ...
    def __init__(
        ...
        super().__init__(D_p, D_t, D_e, unit)
        self.alpha = alpha
        self.beta = beta
    
    def __str__(self):
        return f"ConicalNozzle : D_p = {self.D_p}, D_t = {self.D_t}, D_e = {self.D_e}," +\
            f" \u03B1 = {self.alpha}°, \u03B2 = {self.beta}°, unit = {self.unit}"
        
    @property
    def l(self):
        """ Length of convergent section """
        return (self.D_p-self.D_t)/2/math.tan(math.radians(self.beta))
    ...
    def display(self):
        ...
        # Plenum 
        x = np.linspace(-self.D_p/2,0,10)
        y = np.full_like(x, self.D_p/2)
        ax.plot(x, y, 'k')
        ax.plot(x, -y, 'k')

        # Convergent
        x = np.linspace(0,self.l,10)
        y = self.D_p/2 - x*math.tan(math.radians(self.beta))
        ax.plot(x, y, 'k')
        ax.plot(x, -y, 'k')
        ...
        # Other
        ax.set_xlim(-self.D_p/2,self.l + self.L)
